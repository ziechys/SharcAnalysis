...
def bars_from_file(in_file, out_dir,nmodes=21):
    ...
    for i,bars in enumerate(bar_list):
        ...
        pylab.xlim(left=0.5,right=int(header[0][-1])+0.5)
        pylab.xticks(numpy.arange(1, int(header[0][-1])+1, 1.0),header[1][2:],size='small')
        
        pylab.savefig(out_dir + '/' + bar_titles[i] + '.png', orientation='landscape')
        pylab.savefig(out_dir + '/' + bar_titles[i] + '.pdf', orientation='landscape')
        pylab.tight_layout()
