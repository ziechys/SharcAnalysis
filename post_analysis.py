...
def get_grad(data,nr_traj):
    ...
    try:
        ...
        print(data+"/output.lis:  Error in dataset")
        return
    ...
    for i in range(1,len(gradient)):
        ...
        #print("%8" % str(gradient[i]) + '   ' +  str(step[i]) +  '   ' +str(time[i]) + ' fs')
            if not hop == 'none':
                if float(int(hop)/2) <  errordata[nr_traj][1] and float(int(hop)/2) < time[i]:
                    usable.append(data)
                print(data + '   '  + "%8f" % gradient[i] + '   '  + '%4i' % step[i] + '   '  + '%10f' % time[i]  + ' fs   ' + '%7.2f' % float(int(hop)/2) + '      ' + str(switch).rjust(5)  + '              ' + '%7.2f' % errordata[nr_traj][1] )
                return
            else:
                print(data + '   '  + "%8f" % gradient[i] + '   '  + '%4i' % step[i] + '   '  + '%10f' % time[i]  + ' fs   ' + hop + '      ' + str(switch).rjust(5) + '              ' + '%7.2f' % errordata[nr_traj][1])
                return
        prev = gradient[i]
    ...
    if not hop == 'none':
        ...
        print(data +  '   No doubled gradients              '  + '%7.2f' %  float(int(hop)/2) + '      ' + str(switch).rjust(5) + '              ' + '%7.2f' % errordata[nr_traj][1])
    else:
        print(data +  '   No doubled gradients              '  + hop.rjust(7) + '      ' + str(switch).rjust(5) + '              ' + '%7.2f' % errordata[nr_traj][1])
